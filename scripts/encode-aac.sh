...
if [[ -z $OUTPUT_DIR ]]; then
  ...
  OUTPUT_DIR="$PARENT_DIR/${BASENAME} (AAC)"
fi
...
while IFS= read -r -d '' INPUT_PATH; do
  ...
  if [[ ! $CHANNELS =~ ^[1-9][0-9]*$ ]]; then
    echo "Skipping (no first audio track found): $INPUT_PATH" >&2
    ((skipped += 1))
    continue
  fi

  if [[ $DRY_RUN == true ]]; then
    echo "[Dry run] Would encode '$INPUT_PATH' → '$OUTPUT_PATH'"
    ((processed += 1))
    continue
  fi
  ...
  if ffmpeg -hide_banner -loglevel error -stats \
    ...
    -c:v copy -c:a aac -b:a 192k -ac 2 -c:s copy \
    -map_metadata 0 \
    -map_chapters 0 \
    "$OUTPUT_PATH"; then
